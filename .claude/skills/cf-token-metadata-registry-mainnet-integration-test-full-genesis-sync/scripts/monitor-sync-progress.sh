...
while true; do
  # Fetch one snapshot. Run check-sync-progress.sh's logic inline so we can
  # extract the numbers for delta + ETA without a second SSH round-trip.
  cursor_row=$(ssh -o BatchMode=yes "$REMOTE" \
    "docker exec cf-token-metadata-registry-db-1 psql -U cardano -d cf_token_metadata_registry -tA -c 'SELECT slot, block_number, era FROM cursor_ ORDER BY update_datetime DESC LIMIT 1;'" 2>/dev/null || echo "||")
  ...
  if [[ -z "$cursor_slot" || "$tip_slot" == "0" ]]; then
    ...
    sync_start=${STORE_CARDANO_SYNC_START_SLOT:-65836843}
    ...
    if [[ -n "$prev_slot" ]]; then
      ...
      if (( dslots > 0 && dt > 0 )); then
        ...
        eta_min=$(python3 -c "print(f'{$left/($dslots/$dt)/60:.0f}' if $dslots>0 else 'inf')")
    ...
    if (( UNTIL_TIP )); then
      ...
      if [[ "$reached" == "1" ]]; then
        echo "[monitor] window progress >= 99.95% — exiting."
        exit 0
      fi
    fi
  fi
