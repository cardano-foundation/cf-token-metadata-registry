...
# locally. Pass the three dynamic values as positional args. The remote
# shell uses `eval` once to expand `$HOME` inside REMOTE_PROJECT_DIR.
...
# Ensure github.com host key is trusted (idempotent — sorted/deduped).
mkdir -p "$HOME/.ssh"
