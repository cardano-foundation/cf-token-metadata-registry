...
#   localhost:$LOCAL_API_PORT -> remote:8080  (API)
...
echo "[tunnels] opening: $LOCAL_API_PORT -> 8080 (api), $LOCAL_DB_PORT -> 5432 (db)"
...
# Find the PID of the just-forked ssh process for this REMOTE+ports.
pid=$(pgrep -af "ssh.*-L ${LOCAL_API_PORT}:localhost:8080.*${REMOTE}" | awk '{print $1}' | head -1)
if [[ -z "$pid" ]]; then
...
if curl -fsS --max-time 5 "http://localhost:${LOCAL_API_PORT}/actuator/health" >/dev/null; then
  ...
  echo "[tunnels] /actuator/health NOT yet reachable — that's fine if the API is still booting"
fi
