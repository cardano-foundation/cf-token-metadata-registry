...
public class V2ApiPostgresMetadataIndexer implements V2ApiMetadataIndexer {
      ...
      try {
        final SqlParameterSource params =
            new MapSqlParameterSource(
                Map.ofEntries(entry("pivotid", pivotId), entry("source", metadataSource)));
        final List<MetadataQueryResult> pivotElements =
            namedParameterJdbcTemplate.query(
                "SELECT * FROM metadata WHERE subject = :pivotid AND source = :source",
                params,
  ...
  private Object computePivotCriteriaValue(
      final MetadataQueryResult metadataQueryResult, final String sortingColumnName) {
    return switch (sortingColumnName) {
      ...
      default -> throw new IllegalArgumentException(
          String.format("Invalid sorting column name %s", sortingColumnName));
    };
  }
  ...
  private String whereClauseFromQueryParams(
    ...
    if (!noPageOffsetClause) {
      computePageOffsetClause(
          pivotId, pivotDirection, metadataSource, orderCriteria, sqlParamsSource, filterClauses);
    }
    if (q != null && !q.isBlank()) {
