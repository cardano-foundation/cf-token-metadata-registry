...
public interface V2Api {
    ...
    @Operation(operationId = "deleteSubjectV2", summary = "Delete token metadata.", responses = {@ApiResponse(responseCode = "200", description = "Returns the complete metadata after a successfull request.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TokenMetadata.class))), @ApiResponse(responseCode = "400", description = "Returned if validation of the given metadata object failed.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = VerifyFailureResponse.class)))})
    @RequestMapping(method = RequestMethod.POST, value = "/v2/token/delete", consumes = {"multipart/form-data"})
    ResponseEntity<?> deleteSubjectV2(@ModelAttribute("SubmitTokenDto") SubmitToken submitToken) throws IOException;
    /**
    ...
    @Operation(operationId = "getSubjectsByWallet", summary = "Query all subjects created by wallet address.", responses = {@ApiResponse(responseCode = "200", description = "The metadata related to the queried subjects.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = SubjectsResponse.class))), @ApiResponse(responseCode = "204", description = "If no metadata has been found that matches the given filter criteria.")})
    @RequestMapping(method = RequestMethod.GET, value = "/v2/token/find-by-wallet/{updatedBy}", produces = {"application/json;charset=utf-8"})
    ResponseEntity<List<SubmitToken>> getSubjectsByWallet(@PathVariable String updatedBy);
    ...
    @RequestMapping(method = RequestMethod.POST, value = "/v2/token/submit", consumes = {"multipart/form-data"})
    ResponseEntity<?> postSubjectV2(@ModelAttribute("SubmitTokenDto") SubmitToken submitToken) throws IOException;
